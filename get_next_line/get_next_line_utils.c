...
char	*ft_content_sub(t_list *node, unsigned int start, long len)
{
	...
	if (!str)
		return (0);
	ft_memcpy(str, (node->content) + start, len);
	return (str);
}

static char	*ft_content_concat(t_list *node, char *buf, long res)
{
	char	*tmp;
	long	size;
	...
	{
		ft_memcpy((node->content) + node->length, buf, res);
		node->length += res;
		return (node->content);
	}
	size = node->size * 2;
	...
	if (!tmp)
		return (0);
	ft_memcpy(tmp, node->content, node->length);
	ft_memcpy(tmp + node->length, buf, res);
	free(node->content);
	node->content = tmp;
	node->length += res;
	node->size = size;
